...
def my_account_cogs(ctx, database):
    ...
    message_data = database.select('''SELECT * FROM cantina_hermes.data WHERE token=%s''', (ctx.cookies.get('userID'))
                                , 1)
    try:
        folder_size = check_output(['du', '-sh', user_data[6]]).split()[0].decode('utf-8')
    except CalledProcessError:
        folder_size = 'error'
    return render_template('my_account.html', user_data=user_data, message_data=message_data, folder_size=folder_size)
